...
/**
 * Config input for generating TOML content.
 */
...
/**
 * Generate TOML content from config input.
 *
 * Creates a valid .mastragen/config.toml file content from user-provided settings.
 * Uses defaults for any unspecified values.
 *
 * @param input - Config settings from user
 * @returns TOML file content as string
 */
export function generateConfigToml(input: ConfigGeneratorInput): string {
  const lines: string[] = [
    '# Mastragen Project Configuration',
    '# Generated by mastragen CLI',
    '',
    'version = "1"',
    '',
    '# Phoenix observability - AI trace collection and experiment tracking',
    '[phoenix]',
    `enabled = ${input.phoenix?.enabled ?? true}`,
  ];

  // Add Astro section if enabled
  if (input.astro?.enabled) {
    lines.push(
      '',
      '# Astro UI sandbox - live preview of your Astro frontend',
      '[astro]',
      'enabled = true',
      `path = "${input.astro.path ?? './ui'}"`
    );
  }
