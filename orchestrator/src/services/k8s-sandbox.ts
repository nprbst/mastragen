...
export class K8sSandboxService {
  ...
  /** Cache Phoenix enablement per session for URL generation */
  ...
  async createSandboxPod(
    ...
    // Track Phoenix enablement for URL generation
    if (phoenixConfig?.enabled) {
      this.sessionPhoenixCache.set(session.id, true);
    }
    ...
    // Create Caddyfile ConfigMap (includes Phoenix entry if enabled)
   ...
   */
  private async createCaddyfileConfigMap(
    session: Session,
    ...
    // Build Phoenix Caddyfile entry if enabled
    const phoenixEntry = phoenixConfig?.enabled
      ? `
# Phoenix observability on port ${SANDBOX_PORTS.phoenix} -> internal ${INTERNAL_PORTS.phoenix}
https://${hostname}:${SANDBOX_PORTS.phoenix} {
  tls {
    get_certificate tailscale
  }
  reverse_proxy localhost:${INTERNAL_PORTS.phoenix}
}
`
      : '';
...
${phoenixEntry}`;
  ...
  private buildPodSpec(
    ...
    // Phoenix environment variables for Mastra container (sidecar mode uses 127.0.0.1)
      ...
      spec: {
        ...
        containers: [
          ...
          // T095f, T095i: Caddy sidecar for HTTPS termination (port-based routing)
          {
            ...
            ports: [
              ...
              // Phoenix port added when enabled
              ...(phoenixConfig?.enabled
                ? [{ containerPort: SANDBOX_PORTS.phoenix, name: 'phoenix' }]
                : []),
            ],
            volumeMounts: [
        ...
        volumes: [
          ...
          ...(claudeConfigMapName
            ...
            : []),
          // Phoenix data volume (ephemeral, traces don't need persistence across restarts)
          ...(phoenixConfig?.enabled ? [{ name: 'phoenix-data', emptyDir: {} }] : []),
        ],
      },
    };
  }
